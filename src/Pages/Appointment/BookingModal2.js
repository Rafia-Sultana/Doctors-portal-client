    ...
    fetch('http://localhost:5000/booking', {
      ...
      .then(data => {
        ...
        if (data.success) {
          ...
          toast.error(`You already have an appointment is set on ${data.booking?.date} at${data.booking?.slot}`);
        }
        //to close the modal
        setTreatment(null);
      })
      ...
      <div className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <form onSubmit={handleBooking} className='grid grid-cols-1 gap-3 justify-items-center my-2'>
            <input type="text" disabled value={format(date, 'ppp')} className="input input-bordered w-full max-w-xs" />
            ...
            <input type="text" name="name" placeholder="name" disabled value={user?.displayName || ''} className="input input-bordered w-full max-w-xs" />
